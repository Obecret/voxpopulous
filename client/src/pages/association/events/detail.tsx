...
interface EventWithRegistrations extends AssociationEvent {
...
export default function AssociationEventDetail() {
  const params = useParams<{ slug: string; assocSlug: string; eventId: string }>();
  ...
  const { data: event, isLoading } = useQuery<EventWithRegistrations>({
    queryKey: ["/api/public/associations", association?.id, "events", params.eventId],
  ...
  const registrationMutation = useMutation({
    mutationFn: async (data: { fullName: string; email: string; phone?: string; numberOfGuests: number; comment?: string }) => {
      return apiRequest("POST", `/api/public/associations/${association?.id}/events/${params.eventId}/register`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/public/associations", association?.id, "events", params.eventId] });
  ...
  const { data: eventImages } = useQuery<EventImage[]>({
    queryKey: ["/api/public/associations", association?.id, "events", params.eventId, "images"],
    enabled: !!association?.id && !!event,
  });
    ...
    return (
      <AssociationLayout association={association || null} tenant={tenant || null}>
        <div className="mx-auto max-w-4xl px-4 py-8 text-center">
          ...
          <Link href={`/structures/${params.slug}/${params.assocSlug}/events`}>
