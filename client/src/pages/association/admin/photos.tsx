...
import type { Association, AssociationUser, AssociationPhoto } from "@shared/schema";
...
export default function AssociationAdminPhotos() {
  const params = useParams<{ slug: string; assocSlug: string }>();
  const [, navigate] = useLocation();
  const { toast } = useToast();
  const [selectedPhoto, setSelectedPhoto] = useState<AssociationPhoto | null>(null);
  ...
  const { data: photos, isLoading: photosLoading } = useQuery<AssociationPhoto[]>({
    queryKey: ["/api/associations", data?.association?.id, "admin", "photos"],
  ...
  const createMutation = useMutation({
    mutationFn: async (values: PhotoFormData) => {
      return apiRequest("POST", `/api/associations/${data?.association?.id}/admin/photos`, values);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/associations", data?.association?.id, "admin", "photos"] });
      setIsCreating(false);
      form.reset();
      toast({
        title: "Photo ajoutee",
  ...
  const updateMutation = useMutation({
    mutationFn: async (values: PhotoFormData & { id: string }) => {
      return apiRequest("PATCH", `/api/associations/${data?.association?.id}/admin/photos/${values.id}`, {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (photoId: string) => {
      return apiRequest("DELETE", `/api/associations/${data?.association?.id}/admin/photos/${photoId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/associations", data?.association?.id, "admin", "photos"] });
      setSelectedPhoto(null);
  ...
  const openEditDialog = (photo: AssociationPhoto) => {
    ...
    <AssociationAdminLayout association={association || null} user={user || null} tenantSlug={params.slug}>
      <div className="p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h1 className="font-display text-3xl font-bold" data-testid="text-assoc-photos-title">
              Galerie photos
            </h1>
            <p className="text-muted-foreground mt-1">Gerez les photos de votre association.</p>
          </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input {...field} placeholder="Ex: Entrainement du mardi" data-testid="input-photo-title" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
