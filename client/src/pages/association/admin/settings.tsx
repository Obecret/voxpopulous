...
export default function AssociationAdminSettings() {
  ...
  // Combined query to get both user and association - avoids race condition
  const { data, isLoading: userLoading, error: userError } = useQuery<{ user: SafeAssociationUser; association: Association }>({
  ...
  const { data: settings, isLoading: settingsLoading } = useQuery<SettingsData>({
    queryKey: ["/api/associations", association?.id, "admin", "settings"],
    enabled: !!association?.id && !!user,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: SettingsData) => {
      if (!association?.id) throw new Error("Association non trouvee");
      return apiRequest("PATCH", `/api/associations/${association.id}/admin/settings`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/associations", association?.id, "admin", "settings"] });
      queryClient.invalidateQueries({ queryKey: ["/api/tenants", params.slug, "associations", params.assocSlug] });
  ...
  return (
    <AssociationAdminLayout association={association || null} user={user || null} tenantSlug={params.slug}>
      <div className="p-6">
        <div className="mb-6">
          <h1 className="font-display text-3xl font-bold" data-testid="text-assoc-settings-title">
            Parametres
          </h1>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Presentation de notre association et de ses activites..."
                    rows={4}
