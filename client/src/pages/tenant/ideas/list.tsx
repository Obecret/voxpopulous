...
export default function IdeasList() {
  ...
  const { data: ideas, isLoading } = useQuery<Idea[]>({
    queryKey: ["/api/tenants", params.slug, "ideas"],
  });
      ...
      ideas.forEach(async (idea) => {
        try {
          const response = await fetch(
            `/api/tenants/${params.slug}/ideas/${idea.id}/vote-status?anonymousVoterId=${anonymousVoterId}`
          );
          if (response.ok) {
  ...
  const voteMutation = useMutation({
    mutationFn: async ({ ideaId, voteType }: { ideaId: string; voteType: 'up' | 'down' }) => {
      const anonymousVoterId = getAnonymousVoterId();
      const response = await apiRequest("POST", `/api/tenants/${params.slug}/ideas/${ideaId}/vote`, {
        voteType,
        anonymousVoterId
      });
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Decouvrez les propositions et votez pour celles que vous soutenez.
            </p>
          </div>
                  ...
                  <CardContent className="p-5 flex-1 flex flex-col">
                    ...
                    <div className="flex items-center justify-between mt-4 pt-4 border-t gap-2">
                      ...
                      <Link href={`/structures/${params.slug}/ideas/track/${idea.publicToken}`}>
                        <Button variant="ghost" size="sm" data-testid={`button-track-${idea.id}`}>
                          Voir le suivi
                        </Button>
                      </Link>
                    </div>
