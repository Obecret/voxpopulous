...
import { Search, Filter, Eye, MapPin, Loader2, ImageIcon, Lock, List, Map, Archive, ArchiveRestore } from "lucide-react";
...
export default function AdminIncidents() {
  ...
  const { data: incidents, isLoading } = useQuery<Incident[]>({
    queryKey: ["/api/tenants", params.slug, "admin", "incidents", { showArchived }],
    queryFn: async () => {
      const response = await fetch(`/api/tenants/${params.slug}/admin/incidents?includeArchived=${showArchived}`);
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ incidentId, status }: { incidentId: string; status: string }) => {
      return apiRequest("POST", `/api/tenants/${params.slug}/admin/incidents/${incidentId}/status`, { status });
    },
    onSuccess: () => {
  ...
  const archiveMutation = useMutation({
    mutationFn: async ({ incidentId, isArchived }: { incidentId: string; isArchived: boolean }) => {
      return apiRequest("POST", `/api/tenants/${params.slug}/admin/incidents/${incidentId}/archive`, { isArchived });
    },
  ...
  return (
    <AdminLayout tenant={tenant || null} user={user} electedOfficial={electedOfficial} accountBlocked={accountBlocked} blockReason={blockReason}>
      <div className="p-6">
        <div className="mb-6">
          <h1 className="font-display text-3xl font-bold" data-testid="text-admin-incidents-title">
            Gestion des signalements
          </h1>
          <p className="text-muted-foreground mt-1">
            Traitez les signalements recus.
          </p>
        </div>
