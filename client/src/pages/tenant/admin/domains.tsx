...
import type { Tenant, User, TenantInterventionDomain } from "@shared/schema";
...
export default function AdminDomains() {
  ...
  const [editDomain, setEditDomain] = useState<TenantInterventionDomain | null>(null);
  const [deleteDomain, setDeleteDomain] = useState<TenantInterventionDomain | null>(null);
  ...
  const { data: domains = [], isLoading } = useQuery<TenantInterventionDomain[]>({
  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      return apiRequest("POST", `/api/tenants/${params.slug}/admin/domains`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/tenants", params.slug, "admin", "domains"] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<typeof formData> }) => {
      return apiRequest("PUT", `/api/tenants/${params.slug}/admin/domains/${id}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/tenants", params.slug, "admin", "domains"] });
      setEditDomain(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest("DELETE", `/api/tenants/${params.slug}/admin/domains/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/tenants", params.slug, "admin", "domains"] });
  ...
  const initializeDefaultsMutation = useMutation({
    mutationFn: async () => {
      return apiRequest("POST", `/api/tenants/${params.slug}/admin/domains/initialize-defaults`);
    },
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({ queryKey: ["/api/tenants", params.slug, "admin", "domains"] });
      if (data.created > 0) {
  ...
  const openEdit = (domain: TenantInterventionDomain) => {
    setFormData({
      name: domain.name,
    ...
    <AdminLayout tenant={tenant || null} user={user || null} accountBlocked={user?.accountBlocked} blockReason={user?.blockReason}>
      <div className="p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="font-display text-3xl font-bold" data-testid="text-admin-domains-title">
              Domaines d'intervention
            </h1>
            <p className="text-muted-foreground mt-1">
              Gerez les domaines d'intervention de vos elus.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button onClick={() => { resetForm(); setIsCreateOpen(true); }} data-testid="button-create-domain">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Ex: Urbanisme, Environnement..."
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={formData.name}
                  onChange={(e) => setFormData(f => ({ ...f, name: e.target.value }))}
                  data-testid="input-edit-domain-name"
                />
              </div>
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  value={formData.description}
                  onChange={(e) => setFormData(f => ({ ...f, description: e.target.value }))}
                  data-testid="input-edit-domain-description"
                />
              </div>
              <div className="space-y-2">
                <Label>Couleur</Label>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={formData.displayOrder}
                  onChange={(e) => setFormData(f => ({ ...f, displayOrder: parseInt(e.target.value) || 0 }))}
                  data-testid="input-edit-domain-order"
                />
              </div>
            </div>
            <DialogFooter>
